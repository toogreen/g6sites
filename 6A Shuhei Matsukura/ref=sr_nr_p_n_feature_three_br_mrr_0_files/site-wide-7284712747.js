...
(function(){var plUrls=[],lowPriUrls=[],hiPriUrls=[],isLowPriEligibleYet=false,ST=setTimeout,doc=document,docElem=doc.documentElement,styleObj=docElem.style,nav=navigator,isGecko=("MozAppearance" in styleObj),isWebkit=!isGecko&&("webkitAppearance" in styleObj),isSafari=(isWebkit&&nav.vendor.indexOf("Apple")===0),isIE=!isGecko&&!isWebkit&&nav.appName.indexOf("Microsoft")===0,isMobile=(nav.userAgent.indexOf("Mobile")!=-1),allowedLoaders=((!isMobile&&(isWebkit||isGecko))||(isIE&&typeof XDomainRequest==="object"))?5:2,currentLoaders=0,delay=400,maxDelay=2000;
...
}else{if(u&&!(u[0]==" ")){target.push(u);
}}}}me._getPLStat=function(){return{H:hiPriUrls.length,L:lowPriUrls.length,P:plUrls.length,CL:currentLoaders};
};
...
/* begin navbarJS-jQuery/navbarJQ.js */
;
window.navbar={};
(function(){var navInitJSInteraction=function(){var $=jQuery;
function NavButton(id){var button=this;
this._jqId="#"+id;
this.onShow=function(){button._toggleClass(true,"flyoutOpen");
};
this.onHide=function(){button._toggleClass(false,"flyoutOpen");
};
this.registerTrigger=function(options){var params=this._defaultTriggerParams($(button._jqId),button);
$().extend(params,options||{});
$(button._jqId+" .flyoutAnchor").amazonPopoverTrigger(params);
};
...
this._defaultTriggerParams=function(jQuery,button){return{width:null,location:"bottom",locationAlign:"left",locationMargin:0,hoverShowDelay:0,hoverHideDelay:0,locationElement:jQuery,showOnHover:true,forceAlignment:true,focusOnShow:false,skin:null,additionalCursorRects:jQuery,onShow:button.onShow,onHide:button.onHide,followLink:true,showCloseButton:false};
};
$(button._jqId+" .flyoutAnchor").css({display:"block"});
}var NAV_POPOVER_SKIN='<table cellspacing="0" cellpadding="0" class="nav_popover">'+'<tr class="header"><td class="l navSpritePopoverV"><div class="nav7w"></div></td><td class="c navSpritePopoverH"></td><td class="r navSpritePopoverH"><div class="nav7w"></div></td></tr>'+'<tr class="body"><td class="l navSpritePopoverV"></td><td class="ap_content"></td><td class="r navSpritePopoverV"></td></tr>'+'<tr class="footer"><td class="l navSpritePopoverH"></td><td class="c navSpritePopoverH"></td><td class="r navSpritePopoverV"></td></tr>'+"</table>";
window.initNavSWMPopover=function(options){if(!options){options={};
}var offset=options.offset||[7,-19];
$("a[name^=navSwmSkedPop|]").amazonPopoverTrigger({localContent:"#navSwmSkedPop_content",width:null,location:"bottom",locationAlign:"right",locationMargin:0,locationOffset:offset,hoverShowDelay:0,hoverHideDelay:0,showCloseButton:false,showOnHover:true,forceAlignment:true,focusOnShow:false,skin:NAV_POPOVER_SKIN});
};
var insertBtfFlyoutContents=function(){var contents=_navbar.getBtfFlyoutContents();
;
while(contents.length>0){var c=contents.shift();
var flyoutContent=(typeof(window[c.content])!="undefined")?window[c.content]:c.content;
;
$(flyoutContent).prependTo(document.body);
if(typeof(c.callback)=="function"){c.callback();
}}};
var initYourListsButton=function(){if(!$("#navYourListsButton").length||!$("#listsPop_content").length){return;
}var id="navYourListsButton";
var button=new NavButton(id);
var jQButton=$("#"+id);
var onShow=function(){button.onShow();
$("<div id='navRightButtonMask_listsPop' class='nav_rightButton_mask'><div class='inner navSpritePopoverV'/></div>").css({left:jQButton.offset().left+2,top:jQButton.offset().top+jQButton.outerHeight()-1,width:jQButton.outerWidth()-2}).prependTo(document.body);
};
var onHide=function(){button.onHide();
$("#navRightButtonMask_listsPop").remove();
};
button.registerTrigger({localContent:"#listsPop_content",locationAlign:"right",skin:NAV_POPOVER_SKIN,onShow:onShow,onHide:onHide});
};
var initNavShopAllButton=function(){if(!$("#navShopAllButton").length||!$("#navSaNonGwWrapper").length){return;
}var button=new NavButton("navShopAllButton");
button.registerTrigger({localContent:"#navSaNonGwWrapper"});
};
var navShopAllMenuFlyoutLocation=function($popover,settings){var $trigger=this,$window=$(window),$shopAllBtn=$("#navShopAllButton"),flyoutLeft=$trigger.offset().left+$trigger.outerWidth(),triggerTop=$trigger.offset().top,flyoutHt=$popover.outerHeight(),triggerHt=$trigger.outerHeight(),saButtonBtm=$shopAllBtn.offset().top+$shopAllBtn.height(),winTop=$window.scrollTop(),winHt=$.fn.jquery=="1.2.6"?window.document.compatMode==="CSS1Compat"&&window.document.documentElement["clientHeight"]||window.document.body["clientHeight"]:$window.height(),winBtm=winTop+winHt,flyoutTop=triggerTop,flyoutMoE=5,navAlign="top";
if(triggerTop+flyoutHt>winBtm&&triggerTop>=winTop){if(triggerTop+triggerHt<winBtm){flyoutTop=winBtm-flyoutHt;
navAlign="middle";
}else{flyoutTop=triggerTop+triggerHt-flyoutHt+1;
navAlign="bottom";
if(flyoutTop<saButtonBtm||flyoutTop<winTop){navAlign="middle";
}}}if(navAlign=="middle"){if(flyoutTop<saButtonBtm){flyoutTop=saButtonBtm;
}if(flyoutTop<=winTop){flyoutTop=winTop+1;
}if(triggerTop-flyoutTop<flyoutMoE){flyoutTop=triggerTop;
navAlign="top";
}else{if((flyoutTop+flyoutHt)-(triggerTop+triggerHt)<=flyoutMoE){flyoutTop=triggerTop+triggerHt-flyoutHt+1;
navAlign="bottom";
}}}settings.navAlign=navAlign;
return{"left":flyoutLeft,"top":flyoutTop};
};
var initNavShopAllMenuFlyouts=function(){if(!$("#navSaGwBody.navSaJs").length&&!$("#navSaNonGwWrapper").length){return;
}var onShow=function(popover,settings){var top=this.offset().top,height=this.outerHeight()-2,btmClass="",spriteClass="navSpritePopoverH",edgeTop=0;
switch(settings.navAlign){case"middle":top+=1;
height-=1;
spriteClass="";
break;
case"bottom":top+=1;
height-=1;
btmClass="nav_shop_all_mask_edge_btm ";
edgeTop=height-7+1;
break;
}$("<div id='nav_mask_"+this.attr("id")+"' class='nav_shop_all_mask'></div>").css({top:top,left:this.offset().left+this.outerWidth()+_navbar._saMaskHorizOffset,height:height,"z-index":parseInt(popover.css("z-index"))+1}).append($("<div class='nav_shop_all_mask_edge "+btmClass+spriteClass+"'></div>").css({top:edgeTop})).prependTo(document.body);
var pos=this.attr("class").indexOf("Open");
if(pos==-1){this.attr("class",this.attr("class")+"Open");
}};
var onHide=function(popover,settings){$("#nav_mask_"+this.attr("id")).remove();
var pos=this.attr("class").indexOf("Open");
if(pos!=-1){this.attr("class",this.attr("class").substring(0,pos));
}};
var hoverDelay=(window.navSAFlyoutDelay==null)?0:window.navSAFlyoutDelay;
$("li.navSaMenuItem").each(function(index){$(this).amazonPopoverTrigger({localContent:"#navSaChild_"+this.id.split("_")[1],width:null,location:navShopAllMenuFlyoutLocation,locationMargin:0,locationOffset:[_navbar._saFlyoutHorizOffset,0],hoverShowDelay:hoverDelay,hoverHideDelay:hoverDelay,showCloseButton:false,showOnHover:true,forceAlignment:true,focusOnShow:false,skin:NAV_POPOVER_SKIN,onShow:onShow,onHide:onHide,navAlign:"top"});
});
};
...
if(navDataObject instanceof Object){var closures=new Array();
if(navDataObject.catsubnav){try{var navCatSubnav=$("#navCatSubnav");
if(navCatSubnav.length>0){var catSubnavChanges=0;
var subnav=navDataObject.catsubnav.subnav;
if(subnav&&subnav.type=="linkSequence"){var navSubItem=$("td.navSubItem:first",navCatSubnav);
var navSubMid=$("td.navSubMid:first",navCatSubnav);
if(navSubItem.length==0){throw"subnav-1";
}var newSubnav=[];
for(var i=0;
i<subnav.data.length;
i++){if(i>0&&navSubMid.length>0){newSubnav.push(navSubMid.clone().get(0));
}var datum=subnav.data[i];
if(!datum.href||!datum.text){throw"subnav-2";
}var menuItem=navSubItem.clone();
if(datum.flag){menuItem.removeClass("navSubItem").addClass("navSubItemActive");
}var aElem=$("a:first",menuItem);
if(aElem.length==0){throw"subnav-3";
}aElem.attr("href",datum.href).html(datum.text);
newSubnav.push(menuItem.get(0));
}var navSubLeft=$("td.navSubLeft",navCatSubnav);
if(navSubLeft.length==0){throw"subnav-4";
}if(newSubnav.length>0){closures.push(function(){$("td",navCatSubnav).filter(".navSubMid, .navSubItem, .navSubItemActive").remove();
navSubLeft.after($(newSubnav));
});
catSubnavChanges+=1;
}}var category=navDataObject.catsubnav.category;
if(category&&category.type=="link"){var cDatum=category.data;
if(!cDatum.href||!cDatum.text){throw"category-1";
}var catA=$("td.navCat a:first",navCatSubnav);
if(catA.length==0){throw"category-2";
}closures.push(function(){catA.html(cDatum.text).attr("href",cDatum.href);
});
catSubnavChanges+=1;
}var subnavRowTR=$("#navSubnavRowTR");
var subnavBorderTR=$("#navSubnavBorderTR");
if(subnavRowTR.length>0&&subnavBorderTR.length>0){var shopAllButton=$("#navShopAllButton");
if(shopAllButton.length==0){throw"catsubnav-2";
}if(catSubnavChanges==2){closures.push(function(){subnavRowTR.css("display","");
subnavBorderTR.css("display","");
shopAllButton.addClass("navShopAllButtonSubnav");
});
}else{if(catSubnavChanges==0){closures.push(function(){subnavRowTR.css("display","none");
subnavBorderTR.css("display","none");
shopAllButton.removeClass("navShopAllButtonSubnav");
});
...
}var cartFull=$("#navCartFull");
if(cartFull.length>0){var cartCount=$("#navCartCount");
var cartEmpty=$("#navCartEmpty");
if(cartCount.length==0||cartEmpty.length==0){throw"cart-3";
}closures.push(function(){var cartFullClass="";
if(newCount>0||newCount=="99+"){if(newCount>99){newCount="99+";
cartFullClass="cartFull100";
}else{if(newCount>9){cartFullClass="cartFull10";
}}cartCount.html(newCount);
cartEmpty.css("display","none");
cartFull.removeClass("cartFull10 cartFull100").addClass(cartFullClass).css("display","");
}else{cartCount.html("");
cartFull.css("display","none").removeClass("cartFull10 cartFull100");
...
window.navbar.overrideCartButtonClick=function(clickHandler){$("#navCartEmpty a, #navCartFull a").click(clickHandler);
};
initNavShopAllButton();
initNavShopAllMenuFlyouts();
initYourListsButton();
insertBtfFlyoutContents();
...
if($h.length){$("#navbar").css("marginTop",s.zeroTopMargin?"0":"");
if($.browser.msie){$("#welcomeRowTable").attr("style","").parent().attr("style",s.style||"");
if(parseInt($.browser.version)<8){$("#navLogo").parent().append("<div></div>");
}}else{$("#welcomeRowTable").attr("style",s.style||"");
}$("#navSwmSlot").children().css("display","none");
$h.css("display","");
}});
};
...
/* end navbarJS-jQuery/navbarJQ.js */

/* begin search-js-autocomplete/autocomplete.js */
amznJQ.available("jQuery",function(){(function(window,undefined){var $=window.jQuery,merchRE=/^me=/,refre=/(ref=[-\w]+)/,spaceNormRe=/\s+/g,ddre=/_dd_/,ddaliasre=/(dd_[a-z]{3,4})(_|$)[\w]*/,deptre=/\{department\}/g,slashre=/\+/g,aliasre=/search-alias\s*=\s*([\w-]+)/,nodere=/node\s*=\s*([\d]+)/,noissre=/ref=nb_sb_noss/,dcs="#ddCrtSel",sdpc="searchDropdown_pop_conn",tostr=Object.prototype.toString,ddBox;
$.isArray=$.isArray||function(o){return tostr.call(o)==="[object Array]";
};
var SS=function(sb,pe){var node,div;
function move(){var p=sb.pos(),d=sb.size();
node.css({width:d.width,top:p.top+d.height,left:p.left});
}function create(){var p=sb.pos(),d=sb.size();
node=$("<div id='srch_sggst'/>").css({left:p.left,top:p.top+d.height,width:d.width}).appendTo(pe||sb.parent());
$(window).resize(move);
return div=node.get(0);
}function get(){return div||create();
}this.visible=function(){return get().style.display!="none";
};
this.hide=function(){get().style.display="none";
this.html("");
};
this.show=function(){move();
get().style.display="";
};
this.html=function(h){get().innerHTML=h;
};
this.content=function(c){this.html("");
node.append(c).show();
};
};
var SB=function(sb,h){var curText,hasFocus,curSel,latestUserInput;
init();
...
sb.val(k);
}return $.trim(sb.val().replace(spaceNormRe," "));
...
var AC=function(opts){var version=1,opt={},names,values,crtSel=-1,suggestionList=[],curSize=0,hideDelayTimerId=null,timer=null,maxSuggestions=10,maxCategorySuggestions=4,suggestRequest=null,first=-1,defaultDropDownVal,insertedDropDownVal,delayedDOMUpdate=false,staticContent,searchBox,keystroke,sugHandler,searchSuggest,requestedKeyword="",activityAllowed=true,promoList=[],suggType="sugg",newDesign=$("#navbar").hasClass("nav-beacon"),defaults={sb:"#twotabsearchtextbox",form:"#navbar form[name='site-search']",dd:"#searchDropdownBox",cid:"amazon-search-ui",action:"",sugPrefix:"issDiv",sugText:"Search suggestions",fb:0},lastKeyPressTime,timeToFirstSuggestion=0,searchAliasFrom,defaultTimeout=100;
opts&&init(opts);
...
lookup(opt,"sb");
if(!opt.sb){return;
}searchBox=new SB(opt.sb,{adjust:move,hide:hideSuggestions,dismiss:dismissSuggestions,change:(staticContent?update:delayUpdate),submitEnabled:submitEnabled,keyPress:keyPress,click:clickHandler,newDesign:newDesign});
lookup(opt,"pe");
searchSuggest=new SS(searchBox,opt.pe);
lookup(opt,"form");
lookup(opt,"valInput");
lookup(opt,"dd");
lookup(opt,"submit");
...
}catch(e){}}function clickHandler(kw){if(!kw.length){displayPromotions();
...
timer=null;
}timer=setTimeout(function(){return(function(){if(!kw||!kw.length){displayPromotions();
...
}if(curSize>0&&!newDesign){h+='<div id="sugdivhdr" align="right">&nbsp;</div>';
...
crtSel=parseInt(divId.substr(6));
updateCrtSuggestion();
searchSuggest.hide();
...
if(crtSel>=0){sg=suggestionList[crtSel];
keyword(sg.keyword);
alias=sg.alias;
...
function setDynamicSearch(indexInList,alias){var prefixElems=$("#issprefix");
if(indexInList>=0){var issMode,kw=searchBox.userInput();
if(isFallbackSuggestion(suggestionList,indexInList)){issMode="ss_fb";
}else{if(alias){issMode="ss_c";
}else{if(opt.sc&&isSpellCorrection(suggestionList,indexInList)){issMode="ss_sc";
}else{issMode="ss_i";
}}}setSearchFormReftag(opt.form,null,issMode,indexInList,kw.length);
...
crtSel=parseInt(this.id.substr(6));
...
}function isSpellCorrection(info,index){return info[index].sc=="1";
}function isFallbackSuggestion(info,index){return(info[index].source&&info[index].source[0]=="fb");
...
si=0;
while(combinedList.length<maxSuggestions&&i<n){sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i),conf:extraData[i].conf};
...
sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i),source:extraData[i].source,alias:cs.alias,categoryName:cs.name,conf:extraData[i].conf};
combinedList.push(sg);
++j;
}}++i;
...
return;
}var str=encodeURIComponent(kw),suggestUrl=opt.protocol+"//"+opt.src+"?"+"method=completion"+"&q="+str+"&search-alias="+alias+"&client="+opt.cid+"&mkt="+opt.mkt+"&fb="+opt.fb+"&x=updateISSCompletion";
...
}}function setSearchFormReftag(formElement,optionValue,issMode,indexInList,numUserChars){var formAction=formElement.attr("action"),isstag=(issMode!=null&&indexInList!=null),tag=isstag?issMode+"_"+indexInList+"_"+numUserChars:"dd_"+getShortenedIDForOption(optionValue);
